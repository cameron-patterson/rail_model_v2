...
def rail_model_two_track_e_blocks(section_name, conditions, ex_blocks, ey_blocks, axle_pos_a, axle_pos_b):
    ...
    # "a" first
    v_relay_top_node_a = v_matrix[node_locs_relay_trac_a]
    v_relay_bottom_node_a = v_matrix[node_locs_relay_sig_a]
    v_relay_a = v_relay_top_node_a - v_relay_bottom_node_a

    # "b" first
    v_relay_top_node_b = v_matrix[node_locs_relay_trac_b]
    v_relay_bottom_node_b = v_matrix[node_locs_relay_sig_b]
...
def rail_model_two_track_e_parallel(section_name, conditions, e_parallel, axle_pos_a, axle_pos_b):
    # Create dictionary of network parameters
    parameters = {"z_sig": 0.0289,  # Signalling rail series impedance (ohms/km)
                  "z_trac": 0.0289,  # Traction return rail series impedance (ohms/km)
                  "y_sig_moderate": 0.1,  # Signalling rail parallel admittance for moderate conditions (siemens/km)
...
#ia, ib = rail_model_two_track_e_blocks(section_name="test", conditions="moderate", ex_blocks=np.array([[0, 0], [0, 0], [0, 0]]), ey_blocks=np.array([[0, 0], [0, 0], [0, 0]]), axle_pos_a=np.array([0.99, 1]), axle_pos_b=np.array([2.6]))
