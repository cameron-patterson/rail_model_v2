...
def plot_leak_dif_rs():
    ...
    for i in range(0, len(leak)):
        ...
        lines.append(line)
        labels.append(rf"Y$_{{TR}}$ = {leak[i]} S/km")
...
def plot_leak_rs_current():
    ...
    ax0.set_yticks([0.276, 0.277, 0.278])
    ax0.set_xlim(0, 8.34)
    ...
    ax1.set_yticks([0.048, 0.049, 0.050])
    ax1.set_xlim(0, 8.34)
...
def plot_leak_dif_ws():
    ...
    labels = []
    for i in range(0, len(leak)):
        ia939, ib939 = rail_model_two_track_e_parallel_leak_939_block(leakage=leak[i], resistance=0.0289, e_parallel=e_all, axle_pos_a=(68.34 + axle_spacing), axle_pos_b=np.array([]))
        ia966, ib966 = rail_model_two_track_e_parallel_leak_966_block(leakage=leak[i], resistance=0.0289, e_parallel=e_all, axle_pos_a=(68.34 + axle_spacing), axle_pos_b=np.array([]))
        line, = ax0.plot(e_all, ia939[:, 100])
        ax1.plot(e_all, ia966[:, 100])
