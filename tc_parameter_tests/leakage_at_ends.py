...
def rail_model_two_track_e_parallel(conditions, e_parallel, axle_pos_a, axle_pos_b):
...
def rail_model_two_track_e_parallel_leak_end(conditions, e_parallel, axle_pos_a, axle_pos_b):
    # Create dictionary of network parameters
    parameters = {"z_sig": 0.0289,  # Signalling rail series impedance (ohms/km)
                  "z_trac": 0.0289,  # Traction return rail series impedance (ohms/km)
                  "y_sig_moderate": 0.1,  # Signalling rail parallel admittance for moderate conditions (siemens/km)
...
ia, ib = rail_model_two_track_e_parallel(conditions="moderate", e_parallel=np.array([-10]), axle_pos_a=np.array([50.501]), axle_pos_b=np.array([]))
ia2, ib2 = rail_model_two_track_e_parallel_leak_end(conditions="moderate", e_parallel=np.array([-10]), axle_pos_a=np.array([50.501]), axle_pos_b=np.array([]))
fig, ax = plt.subplots(2, 1)
ax[0].plot(ia[0], '.')
ax[0].plot(ia2[0], '.')
ax[1].plot(ib[0], '.')
ax[1].plot(ib2[0], '.')
ax[0].axhline(0.055, color='tomato')
ax[0].axhline(0.081, color='green')
ax[1].axhline(0.055, color='tomato')
ax[1].axhline(0.081, color='green')
plt.show()
