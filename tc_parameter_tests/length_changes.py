...
def rail_model_two_track_e_parallel_939(mid_block_length, e_parallel, axle_pos_a, axle_pos_b):
...
def rail_model_two_track_e_parallel_966(mid_block_length, e_parallel, axle_pos_a, axle_pos_b):
    # Create dictionary of network parameters
    parameters = {"z_sig": 0.0289,  # Signalling rail series impedance (ohms/km)
                  "z_trac": 0.0289,  # Traction return rail series impedance (ohms/km)
                  "y_sig": 0.1,  # Signalling rail parallel admittance for moderate conditions (siemens/km)
...
def length_currents_0():
    ...
    for i in range(0, len(lengths)):
        ia, ib = rail_model_two_track_e_parallel_939(lengths[i], np.array([0]), np.array([]), np.array([]))
        ia_mid[i] = ia[:, 100]
        print(i)
...
def length_e_field_rs():
    ...
    for i in range(0, len(lengths939)):
        ia939, ib939 = rail_model_two_track_e_parallel_939(lengths939[i], e_fields, np.array([]), np.array([]))
        ax0.plot(e_fields, ia939[:, 100], label=f'{lengths939[i]} km')
        ax0.axhline(0.055, color='tomato', linestyle='--')
        ax0.set_title('BR939A Relay')

        ia966, ib966 = rail_model_two_track_e_parallel_966(lengths966[i], e_fields, np.array([]), np.array([]))
...
def length_e_field_ws():
    ...
    for i in range(0, len(lengths939)):
        axle_pos_939 = 68 + (lengths939[i] / 2)
        ia939, ib939 = rail_model_two_track_e_parallel_939(lengths939[i], e_fields, np.array([axle_pos_939]), np.array([]))
        ax0.plot(e_fields, ia939[:, 100], label=f'{lengths939[i]} km')
        ax0.axhline(0.055, color='lime', linestyle='--')
        ax0.set_title('BR939A Relay')

        axle_pos_966 = 68 + (lengths966[i] / 2)
        ia966, ib966 = rail_model_two_track_e_parallel_966(lengths966[i], e_fields, np.array([axle_pos_966]), np.array([]))
        ax1.plot(e_fields, ia966[:, 100], label=f'{lengths966[i]} km')
        ax1.axhline(0.081, color='lime', linestyle='--')
        ax1.set_title('BR966 F2 Relay')
