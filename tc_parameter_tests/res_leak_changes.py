...
def rail_model_two_track_e_parallel_res_939(resistance, e_parallel, axle_pos_a, axle_pos_b):
    # Create dictionary of network parameters
...
def rail_model_two_track_e_parallel_res_966(resistance, e_parallel, axle_pos_a, axle_pos_b):
    # Create dictionary of network parameters
    parameters = {"z_sig": resistance,  # Signalling rail series impedance (ohms/km)
                  "z_trac": resistance,  # Traction return rail series impedance (ohms/km)
                  "y_sig": 0.1,  # Signalling rail parallel admittance for moderate conditions (siemens/km)
...
def plot_leak_dif_rs():
    ...
    for i in range(0, len(leak)):
        ia939, ib939 = rail_model_two_track_e_parallel_leak_939(leakage=leak[i], e_parallel=e_all, axle_pos_a=np.array([]), axle_pos_b=np.array([]))
        ia966, ib966 = rail_model_two_track_e_parallel_leak_966(leakage=leak[i], e_parallel=e_all, axle_pos_a=np.array([]), axle_pos_b=np.array([]))
...
def plot_leak_dif_ws():
    ...
    thresh966 = np.empty(len(leak))
    for i in range(0, len(leak)):
        ia939, ib939 = rail_model_two_track_e_parallel_leak_939(leakage=leak[i], e_parallel=e_all, axle_pos_a=np.array([68.68]), axle_pos_b=np.array([]))
        ia966, ib966 = rail_model_two_track_e_parallel_leak_966(leakage=leak[i], e_parallel=e_all, axle_pos_a=np.array([68.68]), axle_pos_b=np.array([]))
        ax0.plot(e_all, ia939[:, 100], label=f'Leakage = {leak[i]} S/km')
        thresh939[i] = e_all[np.where(ia939[:, 100] < 0.055)[0][0]]
        ax1.plot(e_all, ia966[:, 100], label=f'Leakage = {leak[i]} S/km')
        thresh966[i] = e_all[np.where(ia966[:, 100] < 0.081)[0][0]]
...
def plot_leak_rs_single():
    ...
    leak = np.linspace(0.242, 7.558, 20)
    leaks939 = np.empty(len(leak))
    leaks966 = np.empty(len(leak))
    for i in range(0, len(leak)):
        print(i)
        ia939, ib939 = rail_model_two_track_e_parallel_leak_939(leakage=leak[i], e_parallel=np.array([-10]), axle_pos_a=np.array([]), axle_pos_b=np.array([]))
        leaks939[i] = ia939[0, 100]
        ia966, ib966 = rail_model_two_track_e_parallel_leak_966(leakage=leak[i], e_parallel=np.array([-10]), axle_pos_a=np.array([]), axle_pos_b=np.array([]))
        leaks966[i] = ia966[0, 100]

    ax0.plot(leak, leaks939)
...
def plot_leak_ws_single():
    ...
    for i in range(0, len(leak)):
        print(i)
        ia939, ib939 = rail_model_two_track_e_parallel_leak_939(leakage=leak[i], e_parallel=np.array([-10]), axle_pos_a=np.array([68.68]), axle_pos_b=np.array([]))
        leaks939[i] = ia939[0, 100]
        ia966, ib966 = rail_model_two_track_e_parallel_leak_966(leakage=leak[i], e_parallel=np.array([-10]), axle_pos_a=np.array([68.68]), axle_pos_b=np.array([]))
        leaks966[i] = ia966[0, 100]
...
def plot_res_rs():
    ...
    for i in range(0, len(res)):
        ia939, ib939 = rail_model_two_track_e_parallel_res_939(resistance=res[i], e_parallel=e_all, axle_pos_a=np.array([]), axle_pos_b=np.array([]))
        ia966, ib966 = rail_model_two_track_e_parallel_res_966(resistance=res[i], e_parallel=e_all, axle_pos_a=np.array([]), axle_pos_b=np.array([]))
        ax0.plot(e_all, ia939[:, 100], label=f'Leakage = {res[i]} ohm/km')
        ax1.plot(e_all, ia966[:, 100], label=f'Leakage = {res[i]} ohm/km')
...
def plot_res_ws():
    ...
    e_all = np.arange(-20, 20.1, 0.01)
    for i in range(0, len(res)):
        ia939, ib939 = rail_model_two_track_e_parallel_res_939(resistance=res[i], e_parallel=e_all, axle_pos_a=np.array([68.68]), axle_pos_b=np.array([]))
        ia966, ib966 = rail_model_two_track_e_parallel_res_966(resistance=res[i], e_parallel=e_all, axle_pos_a=np.array([68.68]), axle_pos_b=np.array([]))
        ax0.plot(e_all, ia939[:, 100], label=f'Resistivity = {res[i]} ohm/km')
        ax1.plot(e_all, ia966[:, 100], label=f'Resistivity = {res[i]} ohm/km')
