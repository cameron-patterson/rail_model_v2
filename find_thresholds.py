...
def e_field_parallel(e_fields, section_name, conditions):
    ...
    # Load in section network sub block angles to calculate number of blocks
    sub_block_angles = np.load("data\\network_parameters\\" + section_name + "\\angles_" + section_name + ".npz")
    n_trac_blocks_a = len(sub_block_angles["trac_angles_a"])
    n_trac_blocks_b = len(sub_block_angles["trac_angles_b"])
    n_sig_blocks_a = len(sub_block_angles["sig_angles_a"])
    n_sig_blocks_b = len(sub_block_angles["sig_angles_b"])
    ...
    # Currents
    i_relays_all_a = np.zeros([len(e_fields), n_sig_blocks_a])
    i_relays_all_b = np.zeros([len(e_fields), n_sig_blocks_a])
    for es in range(0, len(e_fields)):
        # Set up current matrix
        j_matrix = np.zeros(n_nodes)
        e = e_fields[es]

        # "a" first
        e_par_trac_a = np.full(n_trac_blocks_a, e)
        e_par_sig_a = np.full(n_sig_blocks_a, e)
        i_sig_a = e_par_sig_a / parameters["z_sig"]
        i_trac_a = e_par_trac_a / parameters["z_trac"]

        # "b" second
        e_par_trac_b = np.full(n_trac_blocks_b, -e)
        e_par_sig_b = np.full(n_sig_blocks_b, -e)
