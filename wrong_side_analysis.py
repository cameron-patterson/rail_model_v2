...
def wrong_side_two_track(section_name, conditions, exs, eys, axle_pos_a, axle_pos_b):
    ...
    # "a" first
    e_x_par_trac_a = np.outer(exs, np.cos(trac_sb_angles_a))
    e_y_par_trac_a = np.outer(eys, np.sin(trac_sb_angles_a))
    e_par_trac_a = e_x_par_trac_a + e_y_par_trac_a
    e_x_par_sig_a = np.outer(exs, np.cos(sig_sb_angles_a))
    e_y_par_sig_a = np.outer(eys, np.sin(sig_sb_angles_a))
    e_par_sig_a = e_x_par_sig_a + e_y_par_sig_a
    i_sig_a = e_par_sig_a / parameters["z_sig"]
    i_trac_a = e_par_trac_a / parameters["z_trac"]

    # "b" second
    e_x_par_trac_b = np.outer(exs, np.cos(trac_sb_angles_b))
    e_y_par_trac_b = np.outer(eys, np.sin(trac_sb_angles_b))
    e_par_trac_b = e_x_par_trac_b + e_y_par_trac_b
    e_x_par_sig_b = np.outer(exs, np.cos(sig_sb_angles_b))
    e_y_par_sig_b = np.outer(eys, np.sin(sig_sb_angles_b))
...
#ia, ib = wrong_side_two_track("glasgow_edinburgh_falkirk", "moderate", np.array([0]), np.array([0]), np.array([24]), np.array([24]))
